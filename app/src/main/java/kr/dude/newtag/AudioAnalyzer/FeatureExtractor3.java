...
public class FeatureExtractor3 {
    ...
    private static final int FREQ_11 = 0;
    private static final int FREQ_12 = 1; // 4개
    private static final int FREQ_15 = 2; // 4개
    private static final int FREQ_18 = 3; // 4개
    ...
    private static final int END_SAMPLE = 30000;
    ...
    public String getSvmFeature(int channel) throws IOException, ExtractException {
        ...
        // double[] fLeftData = leftData;
        // double[] fRightData = rightData;
        ...
        int[] maxIdxs = new int[1 + 4 + 4 + 4 + 4];

        // 12kHz, 15kHz, 18kHz, 20kHz에 대한 wav피쳐들을 일단 막 담아둘 리스트
        ...
        for (int i = 1; i < 1 + 4 + 4 + 4 + 4; i++) {
            // 좌측
            int maxIdx = getMaxIdx(fLeftData, leftSync + SYNC_LENGTH + 10000 + (EMIT_LENGTH + MUTE_LENGTH) * (i - 1),
                    leftSync + SYNC_LENGTH + 10000 + (EMIT_LENGTH + MUTE_LENGTH) * (i - 1) + 100);
            ...
            // 좌측
            maxIdx = getMaxIdx(fRightData, rightSync + SYNC_LENGTH + 10000 + (EMIT_LENGTH + MUTE_LENGTH) * (i - 1),
                    rightSync + SYNC_LENGTH + 10000 + (EMIT_LENGTH + MUTE_LENGTH) * (i - 1) + 100);
        ...
        String svmFeature = "";
        int prefix = 1;
        int lSvmFeatureIdx = 0;
        int rSvmFeatureIdx = 0;

        int SVM_FEATURE_LEN = 40;
        // 각 주파수에 대해서 MIN, MAX를 새로운 피쳐로 집어넣음
        int ADDITIONAL_FEATURE = 2 * 4;

        double[] lSvmFeature = new double[SVM_FEATURE_LEN + ADDITIONAL_FEATURE];
        // 좌측
        for (int i = 0; i < LfftFeatures.size(); i++) {
            ...
            switch (i + 1) {
                case FREQ_12:
                    startPos = arrTargetFreqSamplePos[FREQ_12] - 5;
                    endPos = arrTargetFreqSamplePos[FREQ_12] + 5;
                    break;
                case FREQ_15:
                    startPos = arrTargetFreqSamplePos[FREQ_15] - 5;
                    endPos = arrTargetFreqSamplePos[FREQ_15] + 5;
                    break;
                case FREQ_18:
                    startPos = arrTargetFreqSamplePos[FREQ_18] - 5;
                    endPos = arrTargetFreqSamplePos[FREQ_18] + 5;
                    break;
                case FREQ_20:
                    startPos = arrTargetFreqSamplePos[FREQ_20] - 5;
                    endPos = arrTargetFreqSamplePos[FREQ_20] + 5;
                    break;
            }
            for (int j = startPos; j < endPos; j++) {
                lSvmFeature[lSvmFeatureIdx] = fftFeature[j];
                lSvmFeatureIdx++;
            }
            lSvmFeature[lSvmFeatureIdx++] = getMin(fftFeature);
            lSvmFeature[lSvmFeatureIdx++] = getMax(fftFeature);

        }

        double[] rSvmFeature = new double[SVM_FEATURE_LEN + ADDITIONAL_FEATURE];
        // 우측
        for (int i = 0; i < RfftFeatures.size(); i++) {
            ...
            for (int j = startPos; j < endPos; j++) {
                rSvmFeature[rSvmFeatureIdx] = fftFeature[j];
                rSvmFeatureIdx++;
            }
            rSvmFeature[rSvmFeatureIdx++] = getMin(fftFeature);
            rSvmFeature[rSvmFeatureIdx++] = getMax(fftFeature);

        }
        double[] mixedSvmFeature = new double[SVM_FEATURE_LEN * 2 + ADDITIONAL_FEATURE * 2];
        for (int i = 0; i < lSvmFeature.length; i++) {
            mixedSvmFeature[i] = lSvmFeature[i];
        }
        for (int i = lSvmFeature.length; i < mixedSvmFeature.length; i++) {
            mixedSvmFeature[i] = rSvmFeature[i - lSvmFeature.length];
        }
    ...
    private double getSum(double[] real, double[] imag, int start, int end) {
        ...
        for (int i = start; i < end; i++) {
            result = result + (Math.sqrt(real[i] * real[i] + imag[i] * imag[i])) / (WINDOW_SIZE / 2);
        }
        return result;
    }
    ...
    private double getAmplitude(double[] real, double[] imag, int idx) {
        double result = 0;
        result = (Math.sqrt(real[idx] * real[idx] + imag[idx] * imag[idx])) / (WINDOW_SIZE / 2);
        return result;
    }
    ...
    private int getSync(double[] input, int start, int end, int gap) throws ExtractException {
        ...
        for (int inputIdx = start; inputIdx < end; inputIdx = inputIdx + gap) {
            ...
            mFftCalculator.fft(real, imag);
            fftcnt++;
            windowSum = getSum(real, imag, arrTargetFreqSamplePos[FREQ_11], arrTargetFreqSamplePos[FREQ_11] + 3);
            // System.out.println("inputIdx :: "+ inputIdx+", windowSum ::
            // "+windowSum);
    ...
    public int getMaxIdx(double[] input, int startIdx, int endIdx) throws ExtractException {
        // System.out.println("getMaxIdx( startIdx :: "+startIdx +", endIdx ::
        // "+endIdx+" )");
        ...
        try {
            ...
            throw new ExtractException("ERROR :: getMaxIdx() is failed - IndexOutOfBoundsException" + ", startIdx :: "
    ...
    public double[] filter(double[] input) {
        ...
        double arrBandPassFilter[] = { 0.001249357, -0.001304802, -0.00219841, 0.004881118, -0.000819208, -0.004996001,
                0.004114648, -0.000967922, 0.003361379, -0.005672403, 0.002138934, -0.000852297, 0.005072683,
                -0.004682537, 2.2408E-05, -0.002266054, 0.00650868, -0.002045592, -0.00125678, -0.004734064,
                0.005473458, 0.002375447, -0.000924853, -0.006295781, 0.000394753, 0.006873842, 0.001118748, -0.004186,
                -0.008238637, 0.008868828, 0.003037482, 0.003821426, -0.017323359, 0.006862, 0.001074169, 0.01740983,
                -0.022096117, 0.002338516, -0.008711342, 0.032364701, -0.018539205, 0.000349932, -0.027775833,
                0.041030837, -0.006080212, 0.008392534, -0.053099953, 0.033853165, 0.011253571, 0.03604806,
                -0.077507428, -0.004581552, 0.026282124, 0.11797702, -0.092551694, -0.280753483, 0.532219261,
                -0.280753483, -0.092551694, 0.11797702, 0.026282124, -0.004581552, -0.077507428, 0.03604806,
                0.011253571, 0.033853165, -0.053099953, 0.008392534, -0.006080212, 0.041030837, -0.027775833,
                0.000349932, -0.018539205, 0.032364701, -0.008711342, 0.002338516, -0.022096117, 0.01740983,
                0.001074169, 0.006862, -0.017323359, 0.003821426, 0.003037482, 0.008868828, -0.008238637, -0.004186,
                0.001118748, 0.006873842, 0.000394753, -0.006295781, -0.000924853, 0.002375447, 0.005473458,
                -0.004734064, -0.00125678, -0.002045592, 0.00650868, -0.002266054, 2.2408E-05, -0.004682537,
                0.005072683, -0.000852297, 0.002138934, -0.005672403, 0.003361379, -0.000967922, 0.004114648,
                -0.004996001, -0.000819208, 0.004881118, -0.00219841, -0.001304802, 0.001249357, };
